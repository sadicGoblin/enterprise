...
export class IncidentReportModalComponent implements OnInit {
  ...
  @ViewChild(IncidentOriginSelectionComponent) originSelection!: IncidentOriginSelectionComponent;
  @ViewChild(IncidentValidationComponent) validationForm!: IncidentValidationComponent;
  ...
  saveReport() {
    ...
    if (!this.validateIncidentForm()) {
      this.validationErrors.push('Complete todos los campos requeridos en la sección "Datos de Reporte"');
    }

    if (!this.validateOriginSelection()) {
      this.validationErrors.push('Seleccione el origen del incidente en la sección "Origen"');
    }

    if (!this.validateCommunication()) {
      this.validationErrors.push('Complete la información de comunicación en la sección "Validación"');
    }
    ...
    try {
      ...
      const validationData = this.validationForm?.getValidationData() || { comunicadoPor: null, comunicadoA: null };
      ...
      const responsableId = this.extractValue(formData.responsableArea || null, 478);
      const responsableNombre = this.personasMap[String(responsableId)] || "FELIPE GALLARDO";
                              
      // Potencial de gravedad (de string a ID numérico)
      const potencialGravedadKey = formData.potencialGravedad as string || 'leve';
      const potencialGravedadId = this.gravedadMap[potencialGravedadKey] || 3;
      const potencialGravedadTxt = this.gravedadTextoMap[potencialGravedadKey] || "LEVE";
      ...
      const tipoIncidenteKey = formData.tipoIncidente as string || 'dano_material';
      const tipoIncidenteId = this.tipoIncidenteMap[tipoIncidenteKey] || 3;
      const tipoIncidenteTxt = this.tipoIncidenteTextoMap[tipoIncidenteKey] || "DAÑO MATERIAL";
      ...
      const trabajadoresTxt = originData.categoriaTextos?.A_TRABAJADORES && originData.categoriaTextos.A_TRABAJADORES.length > 0 ?
                            originData.categoriaTextos.A_TRABAJADORES.join(', ') :
                            '';
      
      const equiposFijosId = 36; // ID FIJO
      const equiposFijosTxt = originData.categoriaTextos?.B_EQUIPOS_FIJOS && originData.categoriaTextos.B_EQUIPOS_FIJOS.length > 0 ?
                            originData.categoriaTextos.B_EQUIPOS_FIJOS.join(', ') :
                            '';
      
      const edificiosId = 37; // ID FIJO
      const edificiosTxt = originData.categoriaTextos?.C_EDIFICIOS && originData.categoriaTextos.C_EDIFICIOS.length > 0 ?
                         originData.categoriaTextos.C_EDIFICIOS.join(', ') :
                         '';
      
      const vehiculosId = 38; // ID FIJO
      const vehiculosTxt = originData.categoriaTextos?.D_VEHICULOS && originData.categoriaTextos.D_VEHICULOS.length > 0 ?
                         originData.categoriaTextos.D_VEHICULOS.join(', ') :
                         '';
      
      const ambientalesId = 39; // ID FIJO
      const ambientalesTxt = originData.categoriaTextos?.E_AMBIENTALES && originData.categoriaTextos.E_AMBIENTALES.length > 0 ?
      ...
      // ID del reportador
      const idReportadoPor = typeof validationData.comunicadoPor === 'string' ? 
      ...
      const requestData = {
        ...
        "situacionObservada": formData.situacionObservada || "SITUACION OBSERVADA",
        ...
        "descripcionAccionRealizar": originData.accion?.descripcion || "DESCRIPCION DE LA ACCION",
        
        // Datos de validación
        "idReportadoPor": idReportadoPor,
        // Usar el nombre directamente o caer al valor predeterminado
        "reportadoPor": (validationData as any).comunicadoPorTexto || this.getPersonaLabelById(validationData.comunicadoPor) || "SEBASTIAN LUNA",
        // Usar el array de nombres si existe, de lo contrario intentar obtener los nombres por ID
        "comunicadoA": (validationData as any).comunicadoATextos && Array.isArray((validationData as any).comunicadoATextos) && 
                       (validationData as any).comunicadoATextos.length > 0 ?
                       (validationData as any).comunicadoATextos.join(", ") :
                       // Fallback al método anterior
                       (Array.isArray(validationData.comunicadoA) && validationData.comunicadoA.length > 0 ?
      ...
      this.proxyService.post('/ws/ReporteIncidenteSvcImpl.php', requestData).subscribe({
        next: (response) => {
          this.isSaving = false;
   ...
   */
  private getPersonaLabelById(id: string | number | null | undefined): string | null {
   ...
   * @param defaultValue Valor por defecto si no se encuentra
   */
  extractFirstValue(options: any[] | null | undefined, defaultValue: any): number {
    if (!options || !Array.isArray(options) || options.length === 0) return defaultValue;
   ...
   * @param defaultLabel Etiqueta por defecto si no se encuentra
   */
  extractFirstLabel(options: any[] | null | undefined, defaultLabel: string): string {
    if (!options || !Array.isArray(options) || options.length === 0) return defaultLabel;
    
    // Intentamos obtener la etiqueta, no el ID
    const firstOption = options[0];
    if (typeof firstOption === 'object' && firstOption !== null && 'label' in firstOption) {
